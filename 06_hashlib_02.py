...
def dump_data(dump_username, dump_password, dump_pin, dump_file_path):
    """
    重编码数据
    :return: 重编码后的字符串
    """
...
def load_data(load_file_path):
    ...
    with open(load_file_path, 'r', encoding='utf-8') as hash_file:
        ...
        hash_pin = hash_data[2].strip()  # 这里可以直接读取pin值
        return [hash_username, hash_password, hash_pin]
...
if __name__ == '__main__':
    ...
    username = 'admin'
    password = '123456'
    user_pin = md5_salt  # 这里的“盐”可以用随机生成的16位字符，也可以用用户的真实密码做“盐”

    file_path = 'data/hashlib_pin.txt'

    # 加密密码
    # 以不同的“用户名”做盐，为用户实现“相对的”动态加密
    encrypted_password = encrypt_keyinfo(md5_salt.encode('utf-8'), password)

    # 存储用户数据
    dump_data(username, encrypted_password, user_pin, file_path)

    # 2.登录阶段
    # 读取用户数据
    [load_username, load_password, load_pin] = load_data(file_path)
    print(f'用户名：{load_username}, 密码：{load_password}, PIN：{load_pin}')

    # 解密密码
    # 重新将密码进行加密，和存入的密码进行比较
    encrypted_password_for_login = encrypt_keyinfo(load_pin.encode('utf-8'), password)
